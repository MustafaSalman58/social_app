...
class _HomeLayoutState extends State<HomeLayout> {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
        ...
        body: StreamBuilder(
          ...
          builder: (ctx, AsyncSnapshot<QuerySnapshot> streamSanpshot) {
            if (streamSanpshot.connectionState == ConnectionState.waiting) {
              return Center(
                child: CircularProgressIndicator(),
              );
            }
            final postDocs = streamSanpshot.data.docs;
            List<PostModel> posts = [];
            List<String> postIds = [];
            List<int> likes = [];
            postDocs.forEach((element) {
  ...
  Widget buildPostItem(BuildContext context, PostModel post, String postId) =>
      Column(
        children: [
          Row(
            children: [
